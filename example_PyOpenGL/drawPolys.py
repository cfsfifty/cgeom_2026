...
state = AppState(FileObj.FileObj(), FileObj.FileObj(), [ -1 ])
...
def drawGeometry():
	...
	point = state.p1.getPointCoords()
	glColor3f  (1.0, 0.0, 0.0)  # Red
	for face in state.p1.getPolygonIndices():
	...
	#glBegin(GL_TRIANGLE_FAN)
	glBegin(GL_LINE_LOOP)
	point = state.p2.getPointCoords()
	glColor3f  (0.0, 1.0, 0.0)  # Red
	for face in state.p2.getPolygonIndices():
		for i, idx in enumerate(face): # Last vertex same as first vertex
			#glColor3f  (random.random(), random.random(), random.random())  # Red
			glVertex2fv(point[idx])
	glEnd()
...
def main():
	state.p1.read("../models/5gon_xdifferent.obj")
	state.p2.read("../models/quad_xdifferent.obj")
