...
def makeTriangulation (face_objects : list[Face]):
    def sameSide (a : bool, b: bool) -> bool:
        return (a and b) or ((not a) and (not b))
    
    addface_objects : list[Face] = list()
    for i, face in enumerate(face_objects):
        face_vertices = face.getHE()
        print(face, len(face_vertices))
        if len(face_vertices) <= 3:
            continue

        keyFunction = lambda he: (he.origin.y, he.origin.x)
        Q = sorted(face_vertices, key=keyFunction, reverse=True)   # oben -> unten
        #assert(not he.sweep is None for he in vertices)
        ...
        for i in range(2, len(Q)-1):
            print(S)
            if (isInteriorRight(S[-1]) and not S[-1] == Q[i].prev) or (not isInteriorRight(S[-1]) and not S[-1] == Q[i].next): # both left side
                Qii = S[-1]
                # insert diagonals
                while len(S) > 1:
                    h1 = insertDiagonal (S[1], Q[i], face_objects)
                    if h1 == face.he or h1 == face.he.next or h1 == face.he.next.next:
                        new_face = Face(h1.twin)
                        addface_objects.append(new_face)
                        face = new_face
                    else:
                        new_face = Face(h1)
                        addface_objects.append(new_face)
                ...
                #assert(S[-2] == S[-1].next and S[-1] == Q[i].next)
                # insert ears
                while len(S) > 1 and isConvexCCW(S[-2].origin, S[-1].origin, Q[i].origin):
                    h1 = insertDiagonal (S[-2], Q[i], face_objects)
                    if h1 == face.he or h1 == face.he.next or h1 == face.he.next.next:
                        new_face = Face(h1.twin)
                        addface_objects.append(new_face)
                        face = new_face
        ...
        #for he in S:
        #    print(he)
        S.pop() # except last (so pop last)
        while len(S) > 1: # except first
            h1 = insertDiagonal (S[-1], Q[-1], face_objects)
            addface_objects.append(Face(h1))
            S.pop()

    face_objects.extend(addface_objects)
    return face_objects
...
def run(filename):
    ...
    triFaces  = makeTriangulation(face_objects)
    print("Triangles:", len(triFaces))

    if not glfw.init():
        sys.exit(1)
    window = glfw.create_window(800, 800, f"Triangulation - {filename}", None, None)
    if not window:
        glfw.terminate()
        sys.exit(1)
