...
void displayCB(GLFWwindow* window);
...
GLuint windowHeight = 200;
char title[] = "OpenGL Tessellation Demo";
...
int main(int argc, char* argv[]) 
{
  ...
  if (!glfwInit()) {
	...
	// GLFW: Create a window and opengl context (version 4.3 core profile).
	glfwWindowHint(GLFW_CONTEXT_VERSION_MAJOR, 4);
	glfwWindowHint(GLFW_CONTEXT_VERSION_MINOR, 3);
	glfwWindowHint(GLFW_OPENGL_PROFILE, GLFW_OPENGL_COMPAT_PROFILE);
    // using OpenGL 4.3 core profile will cause GLU tessellation to fail with error code 100152 (GLU_TESS_ERROR1) because GLU tessellation uses deprecated features such as glBegin() and glEnd(). So we use compatibility profile here.
	//glfwWindowHint(GLFW_OPENGL_PROFILE, GLFW_OPENGL_CORE_PROFILE);
    glfwWindowHint(GLFW_RESIZABLE, GL_TRUE); // default
    glfwWindowHint(GLFW_DOUBLEBUFFER, GL_TRUE); // default
    ...
    glfwSetWindowRefreshCallback(window, displayCB);
	...
	reshapeCB(window, windowWidth, windowHeight);
	...
	while (!glfwWindowShouldClose(window)) {
        //glPollEvents();           // poll for and process events and change state (busy idle processing)
...
///////////////////////////////////////////////////////////////////////////////
// initialize app state (global variables)
///////////////////////////////////////////////////////////////////////////////
bool initAppState()
{
...
void keyboardCB(GLFWwindow* window, int key, int scancode, int action, int mods)
{
    ...
    {
    case 27: // ESCAPE
        glfwSetWindowShouldClose(window, true); // close window and terminate in main loop
        break;
